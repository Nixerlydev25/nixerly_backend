import { z } from "zod";
import { SkillName, JobStatus } from "@prisma/client";
...
export const createJobSchema = z.object({
  ...
  skills: z.array(z.nativeEnum(SkillName)).min(1, "At least one skill is required"),
  requirements: z.string({ required_error: "Job requirements are required" }),
  expiresAt: z.string().datetime("Invalid date format. Please provide a valid ISO date string").optional()
});
